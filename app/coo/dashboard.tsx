...
export default function CooDashboard({ viewer, initialStaff, initialEntries, initialChecklist, initialTargets, initialAudit, month, year }: {
  ...
  return (
    <div className="industrial-grid min-h-screen pb-24 md:pb-10">
      <TopNav role={viewer.role!} userName={viewer.name || viewer.phone || "COO"} onSignOut={signOut} />
      ...
      <main className="mx-auto max-w-7xl px-5 py-6 md:px-8">
        ...
        {tab === "overview" && <section className="space-y-6">
          ...
          <div className="panel overflow-hidden"><div className="border-b border-border px-5 py-4 text-sm font-semibold text-white">Monthly crew summary</div>{sparkies.map((sparky) => { const entry = entries.find((row) => row.sparky_id === sparky.id); return <div key={sparky.id} className="grid grid-cols-[1fr_auto_auto] gap-4 border-b border-border px-5 py-4 text-sm last:border-0"><span>{sparky.name || sparky.phone}</span><span className="text-brand">{entry ? `${entry.score}/100` : "Pending"}</span><span className="w-24 text-right text-green-300">{entry ? money(Number(entry.bonus_earned)) : "-"}</span></div>; })}</div>
        </section>}
        {tab === "entry" && <form onSubmit={saveKpi} className="panel grid gap-4 p-5 md:max-w-3xl md:grid-cols-2">
          <select className="field md:col-span-2" name="sparky_id" required><option value="">Select sparky</option>{sparkies.map((sparky) => <option key={sparky.id} value={sparky.id}>{sparky.name || sparky.phone}</option>)}</select>
        ...
        {tab === "staff" && <div className="panel overflow-hidden">{staff.map((member) => <div key={member.id} className="flex flex-col gap-3 border-b border-border p-4 last:border-0 md:flex-row md:items-center"><div className="flex-1"><p className="font-medium">{member.name || "Pending profile"}</p><p className="text-xs text-text-secondary">{member.phone || "No phone recorded"}</p></div><select value={member.role || ""} onChange={(event) => updateRole(member.id, event.target.value)} className="field !w-auto min-w-40" disabled={member.role === "super_admin" && viewer.role !== "super_admin"}>{roleOptions.map((role) => <option key={role || "none"} value={role}>{role || "No role"}</option>)}</select>{member.id !== viewer.id && <button onClick={() => archiveStaff(member.id)} className="secondary-button !w-auto text-red-300">Archive</button>}</div>)}</div>}
